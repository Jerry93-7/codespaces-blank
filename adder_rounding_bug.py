...
def fp_adder_exp_AgeqB(a_sign, b_sign, out_sign, a_exp, b_exp, out_exp, a_mant, b_mant, out_mant):
    ...
    constraint = And(constraint, Extract(23, 0, res_mant) == op_first(mant_tuple)) # op_first(mant_tuple)
...
def fp_adder_exp_BgeqA(a_sign, b_sign, out_sign, a_exp, b_exp, out_exp, a_mant, b_mant, out_mant):
    ...
    tmp_mant_BgeqA = BitVec('tmp_mant_BgeqA', 24)

    res_sign = BitVec('res_sign', 1)
    res_exp = BitVec('res_exp', 8)
    res_mant = BitVec('res_mant', 25)

    a_mant_check = BitVec('a_mant_check', 24)
    constraint = And(constraint, a_mant_check == a_mant)
    ...
    constraint = And(constraint, tmp_mant_BgeqA == LShR(a_mant, exp_diff_expand))

    mant_res = BitVec('mant_res', 25)
    mant_res_cons = Bool('mant_res_cons')
    cout = BitVec('cout', 1)

    mant_tuple = If(a_sign == b_sign, adder_tuple_wrapper(b_mant, tmp_mant_BgeqA), subtractor_tuple_wrapper(b_mant, tmp_mant_BgeqA))
...
def add_normaliser(in_exp, in_mant):
    ...
    norm_tuple = If(Extract(23, 3, in_mant) == BitVecVal(0b000000000000000000001, 21), norm_sub_shift(in_exp, in_mant, 20),
...
def fp_exp_compare(a_sign, b_sign, non_norm_sign, a_exp, b_exp, non_norm_exp, a_mant_full, b_mant_full, non_norm_mant):
    ...
    ret_constraint = Bool('ret_constraint')
    ret_mant_exp_comp = BitVec('ret_mant_exp_comp', 25)
    ret_exp = BitVec('ret_exp', 8)
    ...
    constraint = And(constraint, ret_mant_exp_comp == second(if_tuple))
    constraint = And(constraint, ret_exp == third(if_tuple))
    constraint = And(constraint, ret_sign == fourth(if_tuple))

    ret_tuple = mkTupleType(constraint, ret_mant_exp_comp, ret_exp, ret_sign)  
...
def fp_adder(a, b):
    ...
    # constraint = And(constraint, norm_first(ovf_tuple))
    # constraint = And(constraint, out_exp == norm_second(ovf_tuple))
    # constraint = And(constraint, out_mant_final == Extract(22, 0, norm_third(ovf_tuple)))
...
if __name__ == "__main__":
    ...
    a_test = BitVecVal(0b00001000010000100010110010010011, 32)
    b_test = BitVecVal(0b00010001011010001001111111011100, 32)
